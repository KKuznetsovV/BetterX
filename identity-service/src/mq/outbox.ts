...
// accompanies - this is the whole point of the outbox pattern: a follow row
// and its corresponding event either both commit or both roll back, so a
// crash or an unreachable RabbitMQ right after the DB commit can no longer
// silently lose the event (the old direct channel.publish() call added in
// earlier steps had exactly that gap).
